...
#akshaya editing
fig3_data <- function(se,dat1, dat2){
    ...
    lesion_size1 <- clindat %>%
      filter(lab_id %in% dat1$lab_id) %>%
      filter(Disease=="HCC") %>%
      select(lab_id, lesion_size) %>%
      set_colnames(c("lab_id", "category")) %>%
      mutate(training_model=model_levels[1])
    lesion_size2 <- clindat %>%
      filter(lab_id %in% dat2$lab_id) %>%
      filter(Disease=="HCC") %>%
      select(lab_id, lesion_size) %>%
      set_colnames(c("lab_id", "category")) %>%
    ...
    category_levels <- c("Non-cancer","Viral Hepatitis",
                         "Cirrhosis",
                         "0", "A", "B", "C","<=1","2-3",">3","<2","2-5",">5")
    clindat3 <- bind_rows(stage1,
                          stage2,
                          nocancer1,
                          nocancer2,lesion_num1,lesion_num2,lesion_size1,lesion_size2) %>%
    ...
    nclevels <- category_levels[1:3]
    stlevels <- category_levels[4:7]
    numlevels <- category_levels[8:10]
    sizelevels <- category_levels[11:13]    #YOU ARE HERE WHEN COME BACK FROM PCR
    combined2 <- combined %>%
        mutate(groups=case_when(category %in% nclevels~"Non-cancer",
                                category %in% stlevels~"Cancer stage",category %in% numlevels~"Lesion Number",category %in% sizelevels~"Lesion Size" ),
               groups=factor(groups, c("Non-cancer", "Cancer stage","Lesion Number","Lesion Size")))    
    combined2
}

fig3_roc <- function(se, dat1, dat2,
                     model_levels){
  category_levels <- c("Non-cancer","Viral Hepatitis",
                       "Cirrhosis",
                       "0", "A", "B", "C")
  ...
  nocancer2 <- clindat %>%
    ...
    roc_strata <- roc_stage2 %>%
        mutate(category=factor(category, category_levels[4:7]))
    roc_strata
}
